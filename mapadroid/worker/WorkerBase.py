...
class WorkerBase(AbstractWorker):
    ...
    def get_devicesettings_value(self, key: str, default_value: object = None):
        ...
        try:
            ...
            self.logger.warning("Failed fetching devicemappings in with description: {}. Stopping worker", str(e))
    ...
    def _internal_pre_work(self):
        ...
        if start_position is not None:
            ...
            self.logger.info('Setting startcoords or walker lat {} / lng {}', str(startcoords[0]), str(startcoords[1]))
        ...
        with self._work_mutex:
            ...
            if restart_pogo_setting > 0:
                ...
                if self._location_count > restart_pogo_setting:
                    self.logger.info("scanned {} locations, restarting game", str(restart_pogo_setting),
                        str(self._origin))
    ...
    def _internal_cleanup(self):
        ...
        self._cleanup()
        self.logger.info("Internal cleanup of {} signaling end to websocketserver")
        ...
        while not self._stop_worker_event.isSet():
            ...
            try:
                ...
                self.logger.error("Websocket connection to lost while running healthchecks, connection terminated "
                    "exceptionally", str(self._origin))
                break
    ...
    def _checkPogoFreeze(self):
        ...
        self.logger.debug4("Old Hash: {}", str(self._lastScreenHash))
        self.logger.debug4("New Hash: {}", str(screenHash))
        if hamming_distance(str(self._lastScreenHash), str(screenHash)) < 4 and str(
            ...
            self.logger.debug("Same Screen Count: {}", str(self._lastScreenHashCount))
    ...
    def _check_pogo_main_screen_tr(self):
        self.logger.debug("_check_pogo_main_screen_tr: Trying to get to the Mainscreen ")
        pogoTopmost = self._communicator.is_pogo_topmost()
        if not pogoTopmost:
            return False
    ...
    def _get_screen_size(self):
        ...
        self.logger.debug('Get Screensize: X: {}, Y: {}, X-Offset: {}, Y-Offset: {}', str(self._screen_x),
                          str(self._screen_y), str(x_offset), str(y_offset))
